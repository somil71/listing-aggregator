...
async function processOne(job) {
  ...
  // Community backfill: if LLM missed the location, scan against the known
  // areas list + fuzzy patterns (Sector X, Phase X, …) from the regex parser.
  if (!parsed.community && !parsed.area_text) {
    const extracted = regexParser.extractLocation(text);
    if (extracted) {
      parsed.community = extracted;
      parsed.area_text = extracted;
      parsed.location  = extracted;
    }
  }
